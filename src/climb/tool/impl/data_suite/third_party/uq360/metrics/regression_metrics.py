...
def auucc_gain(y_true, y_mean, y_lower, y_upper):
    ...
    assert not np.isclose(auucc[1], 0.0), "Failed to calculate auucc gain"
...
def compute_regression_metrics(y_true, y_mean, y_lower, y_upper, option="all", nll_fn=None):
    ...
    if "rmse" in option_list:
        results["rmse"] = root_mean_squared_error(y_true, y_mean)
...
def plot_picp_by_feature(
    ...
    if len(x_uniques_sorted) > 10:  # bin the values
        ...
        picps = np.array(
            [
                picp(
                    y_test[q_sample_bin_ids == bin],
                    y_test_pred_lower_total[q_sample_bin_ids == bin],
                    y_test_pred_upper_total[q_sample_bin_ids == bin],
                )
        ...
        picp_replicated = [
            len(x_uniques_sorted[unique_sample_bin_ids == bin]) * [picps[bin]] for bin in range(num_bins)
        ]
        ...
        picps = np.array(
            [
                picp(
                    y_test[sample_bin_ids == bin],
                    y_test_pred_lower_total[sample_bin_ids == bin],
                    y_test_pred_upper_total[sample_bin_ids == bin],
                )
                for bin in range(num_unique)
            ]
        )
    ...
    if title is None:
        title = "Test data overall PICP: {:.2f} MPIW: {:.2f}".format(
            picp(y_test, y_test_pred_lower_total, y_test_pred_upper_total),
            mpiw(y_test_pred_lower_total, y_test_pred_upper_total),
        )
