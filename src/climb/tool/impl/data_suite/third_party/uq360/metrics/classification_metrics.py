...
def entropy_based_uncertainty_decomposition(y_prob_samples):
    ...
    aleatoric_uncertainty = np.mean(
        np.concatenate([entropy(y_prob, axis=1).reshape(-1, 1) for y_prob in y_prob_samples], axis=1), axis=1
    )
...
def area_under_risk_rejection_rate_curve(
    y_true,
    y_prob,
    y_pred=None,
...
def expected_calibration_error(y_true, y_prob, y_pred=None, num_bins=10, return_counts=False):
    ...
    for bin in range(num_bins):
        ...
        if num_samples_in_bins[bin] > 0:
            accuracies_in_bins[bin] = (
                np.sum(y_true[sample_bin_ids == bin] == y_pred[sample_bin_ids == bin]) / num_samples_in_bins[bin]
            )
            confidences_in_bins[bin] = np.sum(top_scores[sample_bin_ids == bin]) / num_samples_in_bins[bin]

    ece = np.sum(num_samples_in_bins * np.abs(accuracies_in_bins - confidences_in_bins) / num_samples)
...
def plot_reliability_diagram(y_true, y_prob, y_pred, plot_label=[""], num_bins=10):
    ...
    for idx in range(len(plot_label)):
        ece, confidences_in_bins, accuracies_in_bins, frac_samples_in_bins, bins = expected_calibration_error(
            y_true[idx], y_prob[idx], y_pred[idx], num_bins=num_bins, return_counts=True
        )
        ece_list.append(ece)
    ...
    for idx in range(len(plot_label)):
        plt.plot(
            bins, accuracies_in_bins_list[idx], "o-", label="{} ECE = {:.2f}".format(plot_label[idx], ece_list[idx])
        )
...
def plot_risk_vs_rejection_rate(
    y_true,
    y_prob,
    y_pred,
    selection_scores=None,
    ...
    if not isinstance(y_true, list):
        y_true, y_prob, y_pred, selection_scores, subgroup_ids = (
            [y_true],
            [y_prob],
            [y_pred],
            [selection_scores],
    ...
    for idx in range(len(plot_label)):
        plt.plot(
            rejection_rate_list[idx], risk_list[idx], label="{} AURRRC={:.5f}".format(plot_label[idx], aurrrc_list[idx])
        )
