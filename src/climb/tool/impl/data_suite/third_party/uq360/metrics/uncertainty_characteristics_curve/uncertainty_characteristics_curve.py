...
class UncertaintyCharacteristicsCurve:
    ...
    def set_std_unit(self, std_unit=None):
        ...
        if std_unit is None:  # set it to stddev of data
            ...
            if np.isclose(self.std_unit, 0.0):
    ...
    def minimize_cost(
        self, x_axis_cost=0.5, y_axis_cost=0.5, augment_cost_by_normfactor=True, search=("scale", "bias")
    ):
        """
        ...
        if augment_cost_by_normfactor:
            ...
            print(
                "INFO(UCC): Pre-norming costs by corresp. std deviation: new x_axis_cost = %.4f, y_axis_cost = %.4f"
        ...
        for d in range(len(self.d)):
            ...
            original_cost = (
                x_axis_cost * [0.0, m, b, e, df][self.x_axis_idx] + y_axis_cost * [0.0, m, b, e, df][self.y_axis_idx]
            )

            plotdata = self.plotdata_for_scale[d]
            cost_scale, minidx_scale = self._find_min_cost_in_component(
                plotdata, self.x_axis_idx, self.y_axis_idx, x_axis_cost, y_axis_cost
            )
            ...
            if "bias" in search:
                ...
                cost_bias, minidx_bias = self._find_min_cost_in_component(
                    plotdata, self.x_axis_idx, self.y_axis_idx, x_axis_cost, y_axis_cost
                )
    ...
    def get_specific_operating_point(
        self, req_x_axis_value=None, req_y_axis_value=None, req_critical_value=None, vary_bias=False
    ):
        """
        ...
        xnorm = self.std_unit if self.norm_x_axis else 1.0
        ynorm = self.std_unit if self.norm_y_axis else 1.0
        recipe = []
        for dc in range(len(self.d)):
            ...
            recipe.append(
                {
                    "operation": ("bias" if vary_bias else "scale"),
    ...
    def _calc_avg_excess(self, d, lb, ub):
        ...
        excess[posidx] = np.where(ub[posidx] - d[posidx] < 0.0, 0.0, ub[posidx] - d[posidx])
        negidx = np.where(d < 0)[0]
        excess[negidx] = np.where(lb[negidx] + d[negidx] < 0.0, 0.0, lb[negidx] + d[negidx])
    ...
    def _calc_avg_deficit(self, d, lb, ub):
        ...
        posidx = np.where(d >= 0)[0]
        deficit[posidx] = np.where(-ub[posidx] + d[posidx] < 0.0, 0.0, -ub[posidx] + d[posidx])
        negidx = np.where(d < 0)[0]
        deficit[negidx] = np.where(-lb[negidx] - d[negidx] < 0.0, 0.0, -lb[negidx] - d[negidx])
    ...
    def _calc_missrate_bandwidth_excess_deficit(self, d, lb, ub, scale=1.0, bias=0.0):
        ...
        """
        abslband = scale * np.where((lb + bias) < 0.0, 0.0, lb + bias)
        absuband = scale * np.where((ub + bias) < 0.0, 0.0, ub + bias)
    ...
    def _calc_plotdata(self, d, lb, ub, vary_bias=False):
        ...
        for i in range(len(critval)):
            if not vary_bias:
                missrate, bandwidth, excess, deficit = self._calc_missrate_bandwidth_excess_deficit(
                    d, lb, ub, scale=critval[i]
                )
            else:
                missrate, bandwidth, excess, deficit = self._calc_missrate_bandwidth_excess_deficit(
                    d, lb, ub, bias=critval[i]
                )
    ...
    def get_AUUCC(self, vary_bias=False, aucfct="trapz", partial_x=None, partial_y=None):
        ...
        for s in range(len(self.d)):
            ...
            if startt >= endtt - 2:
                ...
                if to_i - from_i < 2:
                    raise RuntimeError(
                        "ERROR(UCC): There are too few samples (1) in the partial-AUC interval specified"
                    )
    ...
    def _get_single_OP(self, d, lb, ub, scale=1.0, bias=0.0):
        """
        ...
        op = [0.0] + [i for i in auxop]  # mimic plotdata (first element ignored here)
        return (op[self.x_axis_idx] / xnorm, op[self.y_axis_idx] / ynorm, xnorm, ynorm)

    def get_OP(self, scale=1.0, bias=0.0):
        """
    ...
    def plot_UCC(
        self, titlestr="", syslabel="model", outfn=None, vary_bias=False, markers=None, xlim=None, ylim=None, **kwargs
    ):
        ...
        for s in range(len(self.d)):
            ...
            if markers is not None:
                marker = markers[s]
            p = plt.plot(
                axisX_data, axisY_data, label=syslabel[s] + (" (AUC=%.3f)" % auucc[s]), marker=marker, **kwargs
            )
            if s + 1 == len(self.d):
                oplab = "OP"
            else:
