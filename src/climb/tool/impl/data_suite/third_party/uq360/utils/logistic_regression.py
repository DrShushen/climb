...
sigmoid = lambda x: 0.5 * (np.tanh(x / 2.0) + 1)
...
class LogisticRegression:
    ...
    def fit(self, weights, init_params, inputs, targets, verbose=True):
        ...
        res = scipy.optimize.minimize(
            fun=training_loss_fun, jac=training_gradient_fun, x0=init_params, tol=1e-6, options={"disp": verbose}
        )
        opt_params = res.x
        if verbose:
    ...
    def compute_jacobian(self, params_one, weights_one, inputs, targets):
        return autograd.jacobian(autograd.jacobian(self.loss, argnum=0), argnum=1)(
            params_one, weights_one, inputs, targets
        ).squeeze()
    ...
    def synthetic_lr_data(N=10000, D=10):
        x = 2.0 * npr.randn(N, D)
        x_test = 1.0 * npr.randn(int(0.3 * N), D)
        w = npr.randn(D, 1)
        y = sigmoid((x @ w)).ravel()
