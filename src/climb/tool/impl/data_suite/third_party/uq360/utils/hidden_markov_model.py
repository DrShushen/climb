...
class HMM:
    ...
    def initialize_params(self, seed=1234):
        ...
        kmeans = KMeans(n_clusters=self.K, random_state=seed, n_init=15).fit(X)
    ...
    def unpack_params(self, params):
        ...
        # log(pi[:-1]) - log(pi[-1]).
        unnorm_A = np.exp(np.append(params[: K**2 - K].reshape(K, K - 1), np.zeros((K, 1)), axis=1))
    ...
    def weighted_loss(self, params, weights):
        ...
        logp = logp + self.weighted_alpha_recursion(
            self.X[0], param_dict["pi0"], param_dict["phi"], self.Precision, param_dict["A"], weights
        )
        return -logp
    ...
    def loss_at_missing_timesteps(self, weights, params):
        ...
        lpx = self.weighted_alpha_recursion(
            self.X[0], param_dict["pi0"], param_dict["phi"], self.Precision, param_dict["A"], weights, store_belief=True
        )
        lpx_alt = self.weighted_beta_recursion(
            self.X[0], param_dict["pi0"], param_dict["phi"], self.Precision, param_dict["A"], weights, store_belief=True
        )
    ...
    def fit(self, weights, init_params=None, num_random_restarts=1, verbose=False, maxiter=None):
        ...
        res = scipy.optimize.minimize(
            fun=training_loss_fun, jac=training_gradient_fun, x0=init_params, tol=1e-10, options=options_dict
        )
        if verbose:
    ...
    def pack_params(params_dict):
        param_list = [
            (np.log(params_dict["A"][:, :-1]) - np.log(params_dict["A"][:, -1])[:, np.newaxis]).ravel(),
    ...
    def get_indices_in_held_out_fold(T, pct_to_drop, contiguous=False):
        ...
        """
        if contiguous:
            l = np.floor(pct_to_drop / 100.0 * T)
            anchor = np.random.choice(np.arange(l + 1, T))
            o = np.arange(anchor - l, anchor).astype(int)
        else:
            # i.i.d LWCV
            o = np.random.choice(T - 2, size=np.int(pct_to_drop / 100.0 * T), replace=False) + 1
        return o

    @staticmethod
    def synthetic_hmm_data(K, T, D, sigma0=None, seed=1234, varainces_of_mean=1.0, diagonal_upweight=False):
        """
