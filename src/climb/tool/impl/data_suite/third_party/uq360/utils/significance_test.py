...
class SignificanceTester:
    ...
    def hypothesis_test(
        self, measurement_1, measurement_2, metric_payload={}, n_iter=1000, tailed="two", verbose=False
    ):
        result = self._permutation_test(
            measurement_1, measurement_2, metric_payload=metric_payload, n_iter=n_iter, tailed=tailed, verbose=verbose
        )
        return result

    def confidence_interval(
        self, measurement, metric_payload={}, metric_kwargs={}, n_iter=10000, alpha=0.05, verbose=False
    ):
        result = self._bootstrap_test(
            measurement,
    ...
    def _bootstrap_test(
        ...
        difference = (sorted_bootstraps[-1] - sorted_bootstraps[0]) / (
            0.5 * (sorted_bootstraps[-1] + sorted_bootstraps[0])
        )
        if difference < 0.001:
            print()
            print()
        ...
        Z_high_corrected = bias_correction + (bias_correction + Z_high) / (
            1.0 - acceleration * (bias_correction + Z_high)
        )
