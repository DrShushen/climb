import numpy as np
from climb.tool.impl.data_suite.third_party.uq360.utils.batch_features.histogram_feature import SingleHistogramFeature
from climb.tool.impl.data_suite.third_party.uq360.utils.batch_features.histogram_utilities import combine_histograms, compute_histogram
from climb.tool.impl.data_suite.third_party.uq360.utils.transformers import PCATransformer
from climb.tool.impl.data_suite.third_party.uq360.utils.transformers.gbm import GBMTransformer
from climb.tool.impl.data_suite.third_party.uq360.utils.transformers.random_forest import RandomForestTransformer
...
class BatchProjection(SingleHistogramFeature):
    ...
    def extract_pointwise_and_batch(self, x: np.ndarray, predictions: np.ndarray, payload: dict):
        vec, histogram, edges = self.extract_features(
            x, predictions, background_hist=np.array(payload["histogram_bins"])
        )
        hist1, hist2, combined_edges = combine_histograms(
            payload["histogram"], histogram, payload["histogram_bins"], edges
        )
