import numpy as np
from climb.tool.impl.data_suite.third_party.uq360.utils.batch_features import BatchFeature
from climb.tool.impl.data_suite.third_party.uq360.utils.batch_features.histogram_utilities import (
...
class HistogramFeature(BatchFeature):
    ...
    def __init__(self, bins, metrics=None):
        ...
        for metric in self.metrics:
            try:
                ...
                print(
                    f"WARNING: Metric: {metric} not in available metrics: {self.available_metrics}. "
    ...
    def compute_distance(self, hist1, hist2):
        ...
        if "wasserstein" in self.metrics:
            bin_centers = [
                0.5 * (self.histogram_edges[i] + self.histogram_edges[i + 1])
                for i in range(len(self.histogram_edges) - 1)
            ]
