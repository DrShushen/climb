...
from climb.tool.impl.data_suite.third_party.uq360.models.bayesian_neural_networks.bnn_models import bayesian_mlp, horseshoe_mlp
...
class BnnRegression(BuiltinUQ):
    ...
    def __init__(self, config, prior="Gaussian"):
        ...
        self.config = config
        if prior == "Gaussian":
            self.net = bayesian_mlp.BayesianRegressionNet(
                ip_dim=config["ip_dim"],
            ...
            self.net = horseshoe_mlp.HshoeRegressionNet(
                ip_dim=config["ip_dim"],
                op_dim=config["op_dim"],
                num_nodes=config["num_nodes"],
                num_layers=config["num_layers"],
    ...
    def fit(self, X, y):
        ...
        for epoch in range(self.config["num_epochs"]):
            ...
            if (epoch + 1) % 10 == 0:
                ...
                print(
                    "Epoch[{}/{}], neg elbo: {:.6f}, noise var: {:.6f}".format(
                        epoch + 1,
                        self.config["num_epochs"],
                        neg_elbo[epoch].item() / X.shape[0],
...
class BnnClassification(BuiltinUQ):
    ...
    def __init__(self, config, prior="Gaussian", device=None):
        ...
        self.device = device
        if prior == "Gaussian":
            self.net = bayesian_mlp.BayesianClassificationNet(
                ip_dim=config["ip_dim"],
                op_dim=config["op_dim"],
                num_nodes=config["num_nodes"],
                num_layers=config["num_layers"],
            ...
            self.net = horseshoe_mlp.HshoeClassificationNet(
                ip_dim=config["ip_dim"],
                op_dim=config["op_dim"],
                num_nodes=config["num_nodes"],
                num_layers=config["num_layers"],
    ...
    def fit(self, X=None, y=None, train_loader=None):
        ...
        for epoch in range(self.config["num_epochs"]):
            ...
            if (epoch + 1) % 10 == 0:
                # print ((net.noise_layer.bhat/net.noise_layer.ahat).data.numpy()[0])
                print(
                    "Epoch[{}/{}], neg elbo: {:.6f}".format(
                        epoch + 1, self.config["num_epochs"], neg_elbo[epoch].item()
                    )
                )
            params_store[epoch] = copy.deepcopy(self.net.state_dict())  # for small nets we can just store all.
        best_model_id = neg_elbo.argmin()  # loss_val_store.argmin() #
