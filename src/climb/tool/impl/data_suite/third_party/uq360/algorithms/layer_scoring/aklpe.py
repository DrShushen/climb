import numpy as np
from climb.tool.impl.data_suite.third_party.uq360.algorithms.layer_scoring.latent_scorer import LatentScorer
from climb.tool.impl.data_suite.third_party.uq360.utils.transformers.nearest_neighbors import BaseNearestNeighbors
...
class AKLPEScorer(LatentScorer):
    ...
    def _fit_test_nn(self, X):
        ...
        for _ in range(self.n_bootstraps):
            ...
            nn_graph = self.nearest_neighbors().fit(X[rand_sub], **self.nearest_neighbors_kwargs)
    ...
    def _compute_null_distribution(self, X):
        split_generator = ShuffleSplit(n_splits=self.n_bootstraps, test_size=0.5, random_state=self.random_state)
