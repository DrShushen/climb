...
class HomoscedasticGPRegression(BuiltinUQ):
    ...
    def __init__(self, kernel=gpytorch.kernels.ScaleKernel(gpytorch.kernels.RBFKernel()), likelihood=None, config=None):
        """
        Args:
    ...
    def fit(self, X, y, **kwargs):
        ...
        self.X_bounds = X_bounds = torch.stack([X.min() * torch.ones(X.shape[1]), X.max() * torch.ones(X.shape[1])])
        ...
        self.inferred_observation_noise = self.scaler.inverse_transform(
            self.model.likelihood.noise.detach().numpy()[0].reshape(1, 1)
        ).squeeze()
    ...
    def predict(self, X, return_dists=False, return_epistemic=False, return_epistemic_dists=False):
        ...
        y_mean, y_lower, y_upper, y_lower_epistemic, y_upper_epistemic = (
            self.scaler.inverse_transform(y_mean.numpy()).squeeze(),
