...
class MetamodelClassification(PostHocUQ):
    ...
    def _get_model_instance(self, model, config):
        ...
        if not all([hasattr(mdl, key) and callable(getattr(mdl, key)) for key in self.callable_keys]):
            raise ValueError(
                "ERROR: Passed model/method failed the interface test. Methods required: %s"
    ...
    def __init__(self, base_model=None, meta_model=None, base_config=None, meta_config=None, random_seed=42):
        ...
        self.base_model = self._get_model_instance(
        ...
        self.meta_model = self._get_model_instance(
            meta_model if meta_model is not None else self.meta_model_default, self.meta_config
        )
    ...
    def fit(
        self, X, y, meta_fraction=0.2, randomize_samples=True, base_is_prefitted=False, meta_train_data=(None, None)
    ):
        """
        ...
        if meta_train_data[0] is None:
            X_base, X_meta, y_base, y_meta = train_test_split(
                X, y, shuffle=randomize_samples, test_size=meta_fraction, random_state=self.random_seed
            )
        else:
    ...
    def predict(self, X):
        ...
        index_of_class_1 = np.where(self.meta_model.classes_ == 1)[0][
            0
        ]  # class 1 corresponds to probab of positive/correct outcome
