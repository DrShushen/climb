...
class AuxiliaryIntervalPredictor(BuiltinUQ):
    ...
    def _main_model_loss(self, y_true, y_pred_mu, y_pred_log_var, y_pred_log_var_aux):
        ...
        # + 0.5 * y_pred_log_var +
        loss = torch.mean(0.5 * torch.exp(-y_pred_log_var) * r**2) + self.config["lambda_match"] * torch.mean(
            torch.abs(torch.exp(0.5 * y_pred_log_var) - torch.exp(0.5 * y_pred_log_var_aux))
        )
        return loss
    ...
    def fit(self, X, y):
        ...
        dataset_loader = DataLoader(TensorDataset(X, y), batch_size=self.config["batch_size"])
            ...
            for epoch in range(self.config["num_main_iters"]):
                ...
                for batch_x, batch_y in dataset_loader:
                    ...
                    main_loss = self._main_model_loss(
                        batch_y, batch_y_pred_mu, batch_y_pred_log_var, batch_y_pred_log_var_aux
                    )
    ...
    def predict(self, X, return_dists=False):
        ...
        X = torch.from_numpy(X).float().to(self.device)
        dataset_loader = DataLoader(X, batch_size=self.config["batch_size"])
