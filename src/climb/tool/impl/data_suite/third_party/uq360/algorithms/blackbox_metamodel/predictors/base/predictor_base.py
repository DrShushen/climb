...
import numpy as np
from climb.tool.impl.data_suite.third_party.uq360.base import Base
    ...
    def _balance_data(self, train_x, train_y):
        ...
        try:
            if num_correct_predictions > num_incorrect_predictions:
                ...
                repeat_num = int(num_correct_predictions / num_incorrect_predictions)
                remaining_num = num_correct_predictions - num_incorrect_predictions * repeat_num
            else:
                true_indices = np.where(train_y == 1)[0]
                supplemental_set_x = train_x[true_indices]
                supplemental_set_y = train_y[true_indices]
                repeat_num = int(num_incorrect_predictions / num_correct_predictions)
