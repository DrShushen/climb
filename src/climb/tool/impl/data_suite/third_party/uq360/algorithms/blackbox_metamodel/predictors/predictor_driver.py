...
import numpy as np
from climb.tool.impl.data_suite.third_party.uq360.algorithms.blackbox_metamodel.predictors.base.predictor_base import PerfPredictor
from climb.tool.impl.data_suite.third_party.uq360.utils.batch_features.blackbox_feature import BlackboxFeature
from climb.tool.impl.data_suite.third_party.uq360.utils.batch_features.drift_classifier import DriftClassifier
from climb.tool.impl.data_suite.third_party.uq360.utils.batch_features.feature_extractor import FeatureExtractor
from climb.tool.impl.data_suite.third_party.uq360.utils.utils import Timer
...
class PredictorDriver(object):
    def __init__(
        ...
        if pointwise_features is not None:
            ...
            if pp_type in ["text_ensemble", "text_ensemble_v1", "text_ensemble_v2"]:
                ...
                self.pointwise_features = pointwise_features_common + [
                    "random_forest",
                    "logistic_regression",
                    "one_class_svm",
                    "umap_kde",
        ...
        if blackbox_features is not None:
            ...
            if pp_type not in ["text_ensemble", "passthrough", "text_ensemble_v1", "text_ensemble_v2"]:
                self.blackbox_features = [
                    "pp_std",
                    "pp_entropy",
                    "pp_bootstrap",
                    "base_model_entropy_ratio",
    ...
    def fit(self, x_train, y_train, x_test, y_test, test_predicted_probabilities=None):
        ...
        test_features, self.payloads = self.feature_extractor.transform_test(
            x_test, predicted_probabilities=test_predicted_probabilities
        )
        ...
        if self.blackbox_features:
            for bb in self.blackbox_features:
                self.blackbox_payloads[bb] = BlackboxFeature.instance(
                    bb, train_labels=self.train_labels
                ).extract_payload(test_predicted_probabilities, y_test)
    ...
    def predict(self, x_prod, predicted_probabilities=None):
        ...
        prod_features, batch_features = self.feature_extractor.transform_prod(
            x_prod, predicted_probabilities, self.payloads
        )
        ...
        if self.blackbox_features:
            for bb in self.blackbox_features:
                ...
                batch_features[bb] = feature.extract_blackbox_feature(
                    predicted_probabilities, predictor_output, self.blackbox_payloads[bb]
                )
