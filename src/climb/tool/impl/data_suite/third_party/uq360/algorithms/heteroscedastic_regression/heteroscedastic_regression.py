...
from climb.tool.impl.data_suite.third_party.uq360.models.heteroscedastic_mlp import GaussianNoiseMLPNet as _MLPNet
...
class HeteroscedasticRegression(BuiltinUQ):
    ...
    def _loss(self, y_true, y_pred_mu, y_pred_log_var):
        return torch.mean(0.5 * torch.exp(-y_pred_log_var) * torch.abs(y_true - y_pred_mu) ** 2 + 0.5 * y_pred_log_var)
