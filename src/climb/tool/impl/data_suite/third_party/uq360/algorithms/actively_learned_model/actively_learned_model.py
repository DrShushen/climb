...
class ActivelyLearnedModel(BuiltinUQ):
    ...
    def __init__(self, config=None, device=None, verbose=True, online=True):
        ...
        self.builtinuqmodel = self.config["model_function"](
            *self.config.get("model_args", ()), **self.config.get("model_kwargs", {})
        )

    def _choose_highest_var_points(self, X, var, K):
        if len(var.shape) == 2:
            pred_dict = {"indices": np.arange(len(var[:, 0])), "var": var[:, 0]}
        else:
            pred_dict = {"indices": np.arange(len(var)), "var": var}
    ...
    def fit(self):
        ...
        for i in range(self.config["T"]):
            ...
            if self.online:
                ...
                X_added = self._choose_highest_var_points(
                    X_proposed, res.y_upper - res.y_lower, self.config["K"]
                )  # use y_upper-y_lower as a proxy of the variance (up to an increasing function)
                ...
                res = self.builtinuqmodel.predict(X_proposed)
                X_added, y_added = self._choose_highest_var_points_and_labels(
                    X_proposed, res.y_upper - res.y_lower, self.config["K"], y_proposed
                )
