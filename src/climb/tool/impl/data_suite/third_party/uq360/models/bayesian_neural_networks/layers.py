...
from climb.tool.impl.data_suite.third_party.uq360.models.bayesian_neural_networks.layer_utils import (
    InvGammaHalfCauchyLayer,
    InvGammaLayer,
)
...
class BayesianLinearLayer(torch.nn.Module):
    ...
    def __init__(self, in_features, out_features, cuda=False, init_weight=None, init_bias=None, prior_stdv=None):
        ...
        if not prior_stdv:
            ...
            self.prior_stdv = torch.FloatTensor([1.0 / np.sqrt(self.weights.size(1))])
        else:
            self.prior_stdv = torch.FloatTensor([prior_stdv])
        # self.prior_stdv = torch.Tensor([1. / np.sqrt(1e+3)])
        self.prior_mean = torch.FloatTensor([0.0])
        # for Bias use a prior of N(0, 1)
        self.prior_bias_stdv = torch.FloatTensor([1.0])
        self.prior_bias_mean = torch.FloatTensor([0.0])
    ...
    def kl(self):
        ...
        kld_weights = (
            self.prior_stdv.log()
            - weights_logvar.mul(0.5)
            + (weights_logvar.exp() + (self.weights.pow(2) - self.prior_mean)) / (2 * self.prior_stdv.pow(2))
            - 0.5
        )
        kld_bias = (
            self.prior_bias_stdv.log()
            - self.bias_logvar.mul(0.5)
...
class HorseshoeLayer(BayesianLinearLayer):
    ...
    def __init__(self, in_features, out_features, cuda=False, scale=1.0):
        super(HorseshoeLayer, self).__init__(in_features, out_features)
        self.cuda = cuda
        self.in_features = in_features
        self.out_features = out_features
        self.nodescales = InvGammaHalfCauchyLayer(out_features=out_features, b=1.0)
...
class RegularizedHorseshoeLayer(HorseshoeLayer):
    ...
    def __init__(self, in_features, out_features, cuda=False, scale=1.0, c_a=2.0, c_b=6.0):
