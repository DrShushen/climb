...
class InvGammaHalfCauchyLayer(torch.nn.Module):
    ...
    def __init__(self, out_features, b):
        ...
        self.ahat = torch.FloatTensor([1.0])  # The posterior parameter is always 1.
    ...
    def initialize_from_prior(self):
        ...
        self.log_sigma.data = torch.FloatTensor(np.random.randn(self.out_features) - 10.0)

    def expectation_wrt_prior(self):
        """
        Computes E[ln p(a^2 | lambda)] + E[ln p(lambda)]
        """
        expected_a_given_lambda = (
            -gammaln(self.const)
            - 0.5 * (torch.log(self.bhat) - torch.digamma(self.ahat))
            + (-0.5 - 1.0) * self.mu
            - torch.exp(-self.mu + 0.5 * self.log_sigma.exp() ** 2) * (self.ahat / self.bhat)
        )
        expected_lambda = (
            -gammaln(self.const)
            - 2 * 0.5 * np.log(self.b)
            + (-self.const - 1.0) * (torch.log(self.bhat) - torch.digamma(self.ahat))
            - (1.0 / self.b**2) * (self.ahat / self.bhat)
        )
    ...
    def fixed_point_updates(self):
        ...
        self.bhat = torch.exp(-self.mu + 0.5 * self.log_sigma.exp() ** 2) + (1.0 / self.b**2)
...
class InvGammaLayer(torch.nn.Module):
    ...
    def expectation_wrt_prior(self):
        ...
        return (
            ...
            - gammaln(self.a)
            + (-self.a - 1) * self.mu
            - self.b * torch.exp(-self.mu + 0.5 * self.log_sigma.exp() ** 2)
        )
