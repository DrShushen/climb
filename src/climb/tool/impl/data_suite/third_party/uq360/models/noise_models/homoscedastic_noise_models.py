...
import numpy as np
import torch
from climb.tool.impl.data_suite.third_party.uq360.models.noise_models.noisemodel import AbstractNoiseModel
...
class GaussianNoiseGammaPrecision(torch.nn.Module, AbstractNoiseModel):
    ...
    def __init__(self, a0=6, b0=6, cuda=False):
        ...
        self.ahat = Parameter(torch.FloatTensor([10.0]))
        self.bhat = Parameter(torch.FloatTensor([3.0]))
    ...
    def loss(self, y_pred=None, y_true=None):
        ...
        return -1 * (
            -0.5 * n * self.const
            + 0.5 * n * (torch.digamma(ahat) - torch.log(bhat))
    ...
    def kl(self):
        ...
        bhat = transform(self.bhat)
        return (
            (ahat - self.a0) * torch.digamma(ahat)
            - torch.lgamma(ahat)
            + gammaln(self.a0)
            + self.a0 * (torch.log(bhat) - np.log(self.b0))
            + ahat * (self.b0 - bhat) / bhat
        )
...
class GaussianNoiseFixedPrecision(torch.nn.Module, AbstractNoiseModel):
    ...
    def __init__(self, std_y=1.0, cuda=False):
    ...
    def loss(self, y_pred=None, y_true=None):
        ...
        :return:
        """
        ll = -0.5 * self.const - np.log(self.sigma_y) - 0.5 * (1.0 / self.sigma_y**2) * ((y_pred - y_true) ** 2)
        return -ll.sum(dim=0)
