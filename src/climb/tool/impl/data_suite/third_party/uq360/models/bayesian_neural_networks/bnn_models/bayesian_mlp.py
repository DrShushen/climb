...
import numpy as np
import torch
from climb.tool.impl.data_suite.third_party.uq360.models.bayesian_neural_networks.layers import BayesianLinearLayer
from climb.tool.impl.data_suite.third_party.uq360.models.noise_models.homoscedastic_noise_models import GaussianNoiseGammaPrecision
...
class BayesianNN(nn.Module, ABC):
    ...
    def __init__(
        self, layer=BayesianLinearLayer, ip_dim=1, op_dim=1, num_nodes=50, activation_type="relu", num_layers=1
    ):
    ...
    def get_weights(self):
        ...
        weight_dict["layerip_logvar"] = torch.cat(
            [self.fc1.weights_logvar, self.fc1.bias_logvar.unsqueeze(1)], dim=1
        ).data.numpy()
        weight_dict["layerop_means"] = torch.cat(
            [self.fc_out.weights, self.fc_out.bias.unsqueeze(1)], dim=1
        ).data.numpy()
        weight_dict["layerop_logvar"] = torch.cat(
            [self.fc_out.weights_logvar, self.fc_out.bias_logvar.unsqueeze(1)], dim=1
        ).data.numpy()
        return weight_dict
...
class BayesianRegressionNet(BayesianNN, ABC):
    ...
    def __init__(
        ...
        self.noise_layer = GaussianNoiseGammaPrecision(a0=6.0, b0=6.0)
    ...
    def mse(self, x, y):
        ...
        """
        E_noise_precision = 1.0 / self.noise_layer.get_noise_var()
...
class BayesianClassificationNet(BayesianNN, ABC):
    ...
    def __init__(
        ...
        super(BayesianClassificationNet, self).__init__(
            layer=layer,
            ip_dim=ip_dim,
            op_dim=op_dim,
            num_nodes=num_nodes,
